...
def plot_token_ranges(results):
    ...
    for idx, obj in enumerate(results):
        ...
        if "min_tokens" in ip and "max_tokens" in ip and "token_count" in obj:
            try:
                min_val = int(ip["min_tokens"])
...
def plot_tokens_by_category(results, category_key):
    ...
    for obj in results:
        ip = obj.get("input_prompt", {})
        if category_key in ip and "token_count" in obj and "min_tokens" in ip and "max_tokens" in ip:
            group = ip[category_key]
            token_count = obj["token_count"]
            try:
                expected_min = int(ip["min_tokens"])
                expected_max = int(ip["max_tokens"])
            except ValueError:
                continue
            grouped_tokens[group].append(token_count)
            grouped_bounds[group].append((expected_min, expected_max))
...
def plot_scatter_reference(results):
    ...
    plt.scatter(indices, expected_min_list, color='green', marker='x', label="Expected Min")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ref", type=str, help="Path to free_results.json", default="/mnt/packages/models/deepseek/janus/tests/free_results.json")
    ...
    # New change: Analyze free test results and print summary
    free_analysis = analyze_free_results(free_data)
    print("Free Analysis Summary:")
    print("----------------------")
    for key, value in free_analysis.items():
        print(f"{key}: {value}")
    
    # Existing analysis for prompt-engineered results
    ...
    # Optionally, plot free test details if desired
    plot_free_results(free_data)
